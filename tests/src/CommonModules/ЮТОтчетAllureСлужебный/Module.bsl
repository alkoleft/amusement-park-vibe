...
Функция СвойстваОтчета(РезультатТеста, Набор, Модуль)
	...
	ЮТЛогирование.Отладка("=== ДИАГНОСТИКА СвойстваОтчета ===");
	ЮТЛогирование.Отладка("Модуль.Метаданные.Имя: " + Модуль.Метаданные.Имя);
	ЮТЛогирование.Отладка("Набор.Представление: " + Набор.Представление);
	ЮТЛогирование.Отладка("РезультатТеста.Метод: " + РезультатТеста.Метод);
	
	// Добавляем свойства из обычного отчета
	Для Каждого Владелец Из Владельцы Цикл
		Если Владелец.Свойство("СвойстваОтчета") Тогда
			ЮТЛогирование.Отладка("Найдены СвойстваОтчета в владельце: " + СтрШаблон(Владелец.СвойстваОтчета));
			ЮТКоллекции.ДополнитьСтруктуру(Свойства, Владелец.СвойстваОтчета);
		Иначе
			ЮТЛогирование.Отладка("НЕ найдены СвойстваОтчета в владельце");
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого Владелец Из Владельцы Цикл
		Если Владелец.Свойство("СвойстваAllure") Тогда
			ЮТЛогирование.Отладка("Найдены СвойстваAllure в владельце: " + СтрШаблон(Владелец.СвойстваAllure));
			ЮТКоллекции.ДополнитьСтруктуру(Свойства, Владелец.СвойстваAllure);
		Иначе
			ЮТЛогирование.Отладка("НЕ найдены СвойстваAllure в владельце");
		КонецЕсли;
	КонецЦикла;
	
	// Проверяем контекст выполнения
	Попытка
		Контекст = ЮТКонтекстСлужебный.КонтекстИсполненияТекущегоУровня();
		Если Контекст.Свойство("СвойстваAllure") Тогда
			ЮТЛогирование.Отладка("Найдены СвойстваAllure в контексте: " + СтрШаблон(Контекст.СвойстваAllure));
			// Добавляем свойства из контекста, если их нет в объектах
			ЮТКоллекции.ДополнитьСтруктуру(Свойства, Контекст.СвойстваAllure);
		Иначе
			ЮТЛогирование.Отладка("НЕ найдены СвойстваAllure в контексте");
		КонецЕсли;
	Исключение
		ЮТЛогирование.Отладка("Ошибка при получении контекста: " + ОписаниеОшибки());
	КонецПопытки;
	
	ЮТЛогирование.Отладка("Итоговые свойства отчета: " + СтрШаблон(Свойства));
	ЮТЛогирование.Отладка("=== КОНЕЦ ДИАГНОСТИКИ СвойстваОтчета ===");
